...
pub fn show_device_info(device: &Device) -> Result<()> {
    ...
    let device_formats = device.enum_formats()
    ...
    for device_format in device_formats {
        ...
        let framesizes = device.enum_framesizes(device_format.fourcc)
            .with_context(|| format!("Failed to enumerate frame sizes for {}", device_format.fourcc))?;
        for framesize in framesizes {
            for discrete in framesize.size.to_discrete() {
                println!("   Size: {}", discrete);

                let frameintervals = device.enum_frameintervals(framesize.fourcc, discrete.width, discrete.height)
                    .with_context(|| format!("Failed to enumerate frame interval for {}@{}", framesize.fourcc, discrete))?;
